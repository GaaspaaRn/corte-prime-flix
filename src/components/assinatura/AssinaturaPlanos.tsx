...
const AssinaturaPlanos = () => {
  ...
  return (
    <section id="planos" className="py-20 px-4 bg-gradient-hero">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {planos.map((plano, index) => (
            <Card 
              key={plano.id}
              className={`relative p-8 bg-gradient-card border transition-all duration-300 hover:shadow-glow/20 animate-fade-in-up ${
                plano.popular 
                  ? 'border-primary/50 shadow-glow/30 scale-105 lg:scale-110' 
            ...
            >
              ...
              <div className="text-center space-y-6">
                ...
                <div className={`inline-flex p-4 rounded-2xl ${
                  plano.popular ? 'bg-primary/20' : 'bg-primary/10'
                }`}>
                  <plano.icon className={`h-8 w-8 ${
                    plano.popular ? 'text-primary' : 'text-primary'
                  }`} />
                </div>
                ...
                <Button 
                  ...
                  className={`w-full font-bold py-6 h-auto rounded-xl transition-all duration-300 ${
                    plano.popular
                      ? 'bg-primary hover:bg-primary/90 text-primary-foreground shadow-glow hover:shadow-glow/80 hover:scale-105'
                      : 'bg-secondary hover:bg-secondary/80 text-secondary-foreground hover:border-primary/50'
                  }`}
                  variant={plano.popular ? 'default' : 'secondary'}
                >
                  {plano.ctaText}
                </Button>
              </div>
        ...
        <div className="mt-16 text-center animate-fade-in-up">
          <p className="text-lg text-muted-foreground mb-4">
            ⚠️ <span className="text-primary font-semibold">Atenção:</span> As vagas são limitadas por região
          </p>
          <p className="text-sm text-muted-foreground">
            Cancele quando quiser • Sem fidelidade • Primeira semana grátis
          </p>
        </div>
      </div>
    </section>
